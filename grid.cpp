...
// Same as the above function, but fills with supplied data in cell_init
int cd_grid_create_fill(struct cd_grid ** gp,
   void * cell_init, int cell_size, int n, ...)
{
   size_t i;
   va_list argp;
   ...
   for (i=0; i<n; i++)
      sizes[i] = va_arg(argp, int);
   va_end(argp);
   return cd_grid_create_sizeown_fill(gp, cell_init, cell_size, n, sizes);
}

int cd_grid_create_sizeown_fill(struct cd_grid ** gp,
   void * cell_init, int cell_size, int n, int * sizes)
{
   size_t i;
   struct cd_grid * g;
   ...
   if (cell_size > 0)
   {
      ...
      /* Initialize cell data */
      //for (i=0; i<g->ncells; i++)
      memcpy(g->data, cell_init, g->ncells*cell_size);
   }
   /* Allocate lengths vector */
   g->lengths = (double *) malloc(n*sizeof(double));
   if (!g->lengths) { cd_grid_destroy(g); return -1; }
...
// };
   ...
   //printf("After index: %lu, value = %lf\n", index, value);
   subs = (int *) malloc((g->n+1) * sizeof(int));
   cd_grid_index_to_subs(g,index, subs);
   //printf("Subscript: [%d, %d, %d]\n", subs[0], subs[1], subs[2]);
