...
// Eric's implementation, which allows you to initialize the grid with the full contents (could do this
// element by element, but would require going directly into the g->data structure. Would be more efficient...)
int cd_grid_create_fill(struct cd_grid ** gp, void * cell_init, int cell_size, int n, ...);
int cd_grid_create_sizeown_fill(struct cd_grid ** gp, void * cell_init, int cell_size, int n, int * sizes);
